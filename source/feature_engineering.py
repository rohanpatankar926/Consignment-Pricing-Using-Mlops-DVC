...
class FeatureEngineering:
    ...
    def data_(self,config_path):
        self.config=self.get_data.read_params("H:/consignment pricing using mlops/params.yaml")
    ...
    def outlier_detection(self,data,colname):
        self.data=data[data[colname]<=(data[colname].mean()+3*data[colname].std())]
        return self.data
    ...
    def remove_outliers(self):
        try:
            ...
            self.data=self.data_("H:/consignment pricing using mlops/params.yaml")
            self.data0=self.outlier_detection(self.data,"line_item_value")
            self.data1=self.outlier_detection(self.data0,"unit_of_measure_(per_pack)")
            ...
            self.logger.log(logfile,"removed outliers function compiled successfully")
            return self.data
        except Exception as e:
            self.logger.log(logfile,"Exception occured in remove_outliers method"+str(e))
